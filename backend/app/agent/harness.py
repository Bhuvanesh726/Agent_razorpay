...
def handle_confirm(
    ...
    result = _execute_tool(db, user_id, tool_name, arguments)
    agent_session_repo.append_message(
...
def _handle_tool_call(db: Session, session, tc: GatewayToolCall, request_id: str | None) -> HarnessResult | None:
    ...
    # ALLOW
    result = _execute_tool(db, user_id, tc.name, args)
    agent_session_repo.append_message(
...
def _execute_tool(db: Session, user_id: str, tool_name: str, args: dict) -> dict:
    try:
        return TOOL_FUNCTIONS[tool_name](db, user_id, **args)
